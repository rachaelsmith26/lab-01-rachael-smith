...
When you enter this and hit enter it asks you to enter your password.
However what this really wants is a personal access token and if you just enter your password you see the message below:
...
Scroll down to where is says developer settings on the left hand side and click this.
Then you will see the following menu below where you will click ***Tokens (classic)***.

```{r git-personal-token-3, fig.fullwidth=TRUE, echo = FALSE}
knitr::include_graphics("img/git-personal-token-3.png")
```

Once you have done this click ***Generate a personal access token***.
...
Scroll to the bottom of this page and click to finish.
You then have generated a personal access token.

```{r git-personal-token-5, fig.fullwidth=TRUE, echo = FALSE}
knitr::include_graphics("img/git-personal-token-5.png")
```

Copy this token and go back to the previous directions about using the ***push*** command.
When prompted enter your github username and then when prompted for password paste the token we just generated in.
...
There are 1846 rows and 3 variables.
...
The r of dino is just slightly higher than the r of star by 0.0015
...
The r of dino is less than the r of circle by 0.0038
